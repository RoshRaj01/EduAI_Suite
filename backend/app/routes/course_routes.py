...
def _enrich_course(course: Course, db: Session) -> dict:
    """Return course as dict with live-computed students count and progress."""
    student_count = db.query(Student).filter(Student.course_id == course.id).count()
    ...
    if total_slots > 0:
        submission_ids = [a.id for a in assignments]
        received = db.query(Submission).filter(
    ...
    d = {
        col.name: getattr(course, col.name)
